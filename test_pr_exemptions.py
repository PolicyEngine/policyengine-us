...
assert (
    personal_exemption == expected_personal
), f"Expected ${expected_personal:,.0f}, got ${personal_exemption:,.0f}"

# Test dependent eligibility
student_eligible = sim.calculate("pr_eligible_dependent_for_exemption", 2024)[
    2
]
young_eligible = sim.calculate("pr_eligible_dependent_for_exemption", 2024)[3]
veteran_eligible = sim.calculate("pr_eligible_dependent_for_exemption", 2024)[
    4
]
...
assert (
    dependent_exemption == expected_dependents
), f"Expected ${expected_dependents:,.0f}, got ${dependent_exemption:,.0f}"

# Test veteran exemption (person-level)
veteran_exemption_person = sim.calculate("pr_veteran_exemption", 2024)[
    4
]  # 5th person (index 4) is the veteran
expected_veteran = 1_500  # 1 veteran at $1,500
print(f"Veteran Exemption (person): ${veteran_exemption_person:,.0f}")
assert (
    veteran_exemption_person == expected_veteran
), f"Expected ${expected_veteran:,.0f}, got ${veteran_exemption_person:,.0f}"
...
assert (
    personal_separate == 3_500
), f"Expected $3,500, got ${personal_separate:,.0f}"
...
assert (
    dependent_separate == 1_250
), f"Expected $1,250, got ${dependent_separate:,.0f}"
