...
parameters, and interactions of major programs including the Earned Income Tax Credit (EITC), 
Child Tax Credit (CTC), Supplemental Nutrition Assistance Program (SNAP), Unemployment Insurance (UI), 
and numerous other federal and state programs. The database provides researchers, policymakers, and 
financial institutions with detailed, machine-readable policy rules that can be used for distributional 
analysis, reform modeling, and understanding program interactions. All parameters are traced to primary 
legislative and regulatory sources, with historical values from 2017 forward and projections under 
current law.
\end{abstract}
...
"""
    ...
    def markdown_to_latex(self, content: str) -> str:
        ...
        for char in ['%', '&', '_']:
            if char != '_' or not re.match(r'\\[a-zA-Z]+{[^}]*' + char, content):
    ...
    def generate_paper(self, include_sections: List[str] = None):
        ...
        if include_sections is None:
            include_sections = [
                'policy/federal/taxation/income/credits/eitc.md',
                'policy/federal/taxation/income/credits/ctc.md',
                'policy/federal/transfers/safety-net/snap.md',
                'policy/federal/transfers/social-insurance/unemployment.md'
            ]
...
\item Congressional Research Service. (2024). \textit{The Earned Income Tax Credit (EITC): 
How It Works and Who Receives It}. Report R43805. 
\url{https://crsreports.congress.gov/product/pdf/R/R43805}

\item Internal Revenue Service. (2024). \textit{Publication 596: Earned Income Credit}. 
Department of the Treasury. \url{https://www.irs.gov/pub/irs-pdf/p596.pdf}

\item Tax Policy Center. (2024). \textit{Briefing Book: Key Elements of the U.S. Tax System}. 
Urban Institute and Brookings Institution. 
\url{https://www.taxpolicycenter.org/briefing-book}

\item U.S. Department of Agriculture. (2024). \textit{Supplemental Nutrition Assistance Program 
(SNAP) Policy Database}. Economic Research Service. 
\url{https://www.ers.usda.gov/data-products/snap-policy-data-sets/}

\item U.S. Department of Labor. (2024). \textit{Unemployment Insurance Program Letter Series}. 
Employment and Training Administration. 
\url{https://wdr.doleta.gov/directives/}

\item U.S. Government Accountability Office. (2023). \textit{Federal Low-Income Programs: 
Eligibility and Benefits Differ for Selected Programs}. Report GAO-23-104577. 
...
"""
    ...
    def _compile_pdf(self, tex_file: Path):
        ...
        try:
            ...
            print("pdflatex not found. Install a LaTeX distribution to compile PDF.")
