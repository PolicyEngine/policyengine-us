...
class VariableDocGenerator:
            ...
            for name, obj in inspect.getmembers(module):
                if (
                    ...
                    metadata = {
                        ...
                        "entity": (
                            getattr(obj, "entity", "").__name__
                            if hasattr(obj, "entity")
                            else ""
                        ),
                        "definition_period": getattr(
                            obj, "definition_period", ""
                        ),
                        "value_type": (
                            getattr(obj, "value_type", "").__name__
    ...
    def generate(self):
        ...
        print(
            f"Found {sum(len(v) for v in self.variables_by_module.values())} variables in {len(self.variables_by_module)} modules"
        )
