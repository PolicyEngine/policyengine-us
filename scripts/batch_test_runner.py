...
def cleanup_memory():
    ...
    if get_memory_usage() > 2000:  # If over 2GB
        print(
            f"  High memory detected ({get_memory_usage():.1f} MB), forcing cleanup..."
        )
        ...
        for variable, expected in outputs.items():
            try:
                actual = simulation.calculate(
                    variable, test_data.get("period", "2024")
                )
                ...
                if abs(float(actual) - float(expected)) > 0.01:
                    passed = False
                    failures.append(
                        f"{variable}: expected {expected}, got {actual}"
                    )
            ...
            if result["passed"]:
                ...
                if result["failures"]:
                    ...
                    results["errors"].append(
                        {"test": test_name, "failures": result["failures"]}
                    )
...
def batch_run_tests(
    test_files: List[str], batch_size: int = 10, cleanup_frequency: int = 5
):
    """
    ...
    for batch_num, i in enumerate(range(0, total_files, batch_size)):
        ...
        for file_path in batch:
            ...
            if result["failed"] == 0:
                print(
                    f" ✓ ({result['passed']}/{result['total']}) in {file_time:.1f}s"
                )
            else:
                print(
                    f" ✗ ({result['passed']}/{result['total']}) in {file_time:.1f}s"
        ...
        if (batch_num + 1) % cleanup_frequency == 0:
            ...
            print(
                f"  Memory after cleanup: {after_memory:.1f} MB (freed {current_memory - after_memory:.1f} MB)"
            )
    ...
    if SYSTEM_INIT_TIME:
        ...
        print(
            f"Amortized init cost per file: {SYSTEM_INIT_TIME/total_files:.2f}s"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Batch test runner for PolicyEngine"
    )
    parser.add_argument(
        "--path",
    ...
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Number of files per batch"
    )
    parser.add_argument(
        "--cleanup-frequency",
        type=int,
    ...
    parser.add_argument(
        "--limit", type=int, help="Limit number of files to test"
    )
