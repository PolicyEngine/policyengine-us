...
def run_test_batch(test_files: List[str], batch_name: str) -> Dict[str, Any]:
    ...
    test_list_file = f"/tmp/contrib_test_batch_{batch_name.replace('/', '_')}.txt"
    ...
    # Run tests
    cmd = [
        python_exe, "-m", "policyengine_core.scripts.policyengine_command",
        "test", *test_files, "-c", "policyengine_us"
    ]
...
def main():
    ...
    print(f"Found {total_files} test files in {len(test_categories)} categories")
    ...
    for category in sorted(test_categories.keys()):
        ...
        for i in range(0, len(test_files), batch_size):
            ...
            if not result["success"]:
                ...
                print(f"✅ Batch {batch_name} passed in {result['elapsed_time']:.2f}s")
    ...
    with open(results_file, 'w') as f:
        json.dump({
            ...
            "summary": {
                ...
                "categories": {k: len(v) for k, v in test_categories.items()}
            },
