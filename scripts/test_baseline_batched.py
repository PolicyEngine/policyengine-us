...
def run_batch_isolated(test_dirs, timeout_seconds):
    ...
    try:
        ...
        cmd = [python_exe, "-m", "policyengine_core.scripts.policyengine_command", 
               "test"] + test_dirs_list + ["-c", "policyengine_us"]
        ...
        print(f"    Completed in {elapsed:.1f}s: {passed_count} passed, {failed_count} failed")
    ...
    if states_path.exists():
        for state_dir in sorted(states_path.iterdir()):
            if state_dir.is_dir():
                ...
                if list(state_dir.glob("*.yaml")) or list(state_dir.rglob("*.yaml")):
...
def run_tests_in_batches(test_files, batch_size=None, timeout_per_batch=1200):
    ...
    conservative = initial_memory > 500  # Use conservative if already using >500MB
    
    if conservative:
        print("Using conservative batching strategy (more batches, less memory)")
    else:
    ...
    for dir_path in parent_dirs:
        ...
        if uncovered_files:
            ...
            already_covered_paths.update(test_files)  # Mark all files in this dir as covered
    
    total_tests = sum(count for _, count in valid_batches)
    print(f"Running {total_tests} tests in {len(valid_batches)} parent directory batches")
    print("-" * 80)
    ...
    for batch_idx, (dir_path, test_count) in enumerate(valid_batches, 1):
        ...
        print(f"  Memory: {current_memory:.1f} MB (growth: +{memory_growth:.1f} MB from start)")
        print(f"  Running total: ✓ {passed} | ✗ {failed} | ⏱ {timeouts} | ❌ {errors}")
        ...
        print(f"  Memory after cleanup: {cleaned_memory:.1f} MB (freed: {memory_freed:.1f} MB)")
    ...
    if memory_stats:
        ...
        for stat in memory_stats:
            print(f"{stat['batch']:30} Peak: {stat['peak_memory']:6.1f} MB, Freed: {stat['memory_freed']:5.1f} MB")
    
    return detailed_results, {"passed": passed, "failed": failed, "timeouts": timeouts, "errors": errors}
...
def main(batch_size=None):
    ...
    """
    print("PolicyEngine Baseline Test Runner - Optimized Parent Directory Batching")
    print("=" * 80)
    print("Strategy: Run parent directories to ensure single initialization per batch")
