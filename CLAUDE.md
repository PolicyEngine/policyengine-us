...
- **Debugging Entity Relationships**:
  ...
  **CHIP Eligibilty**
    - Write python code that returns a person's Eligibilty for the CHIP program
    - this will either be a child (it is the children's health insurance program) or a pregnant woman
    - model the code based on the ACA and Medicaid eligibily variables
    - these are found here : policyengine_us/variables/gov/hhs/medicaid/eligibility and here: policyengine_us/variables/gov/aca/eligibility
    - i have made some standin files but feel free to add stuff if needed
    
   **Rules**
    - use parameterized functions. parameters are here policyengine_us/parameters/gov/hhs/chip
    - if a parameter is needed that does not exist, either make it or ask for help
    - people are eligible for chip only when they made too much money to qualify for medicaid
        - this will either be a child (it is the children's health insurance program) or a pregnant woman
    - this is a person level variable as each memeber of a household can qualify for different programs
