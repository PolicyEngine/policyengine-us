...
def create_rent_relief_tax_credit() -> Reform:
    class rent_relief_tax_credit(Variable):
        ...
        def formula(tax_unit, period, parameters):
            ...
            p = parameters(period).gov.contrib.harris.rent_relief_act.rent_relief_credit
            ...
            gross_income_fraction = p.rent_income_share_threshold * gross_income
            rent_excess = max_(capped_rent - gross_income_fraction, 0)
            safmr_used_for_hcv = tax_unit.household("safmr_used_for_hcv", period)
            high_income_reduction = (
                safmr_used_for_hcv * p.high_income_area_threshold_increase
            )
            applicable_gross_income = max_(gross_income - high_income_reduction, 0)
            ...
            housing_assistance = tax_unit.spm_unit("housing_assistance", period)
...
def create_rent_relief_tax_credit_reform(parameters, period, bypass: bool = False):
    if bypass:
...
rent_relief_tax_credit = create_rent_relief_tax_credit_reform(None, None, bypass=True)
