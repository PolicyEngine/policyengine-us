...
class mi_surtax(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        in_effect = parameters(period).gov.contrib.states.mi.surtax.rate.in_effect
        if not in_effect:
            return 0
        ...
        if filing_status == FilingStatus.JOINT:
            surtax_params = parameters(period).gov.contrib.states.mi.surtax.joint
        else:
            surtax_params = parameters(period).gov.contrib.states.mi.surtax.single
