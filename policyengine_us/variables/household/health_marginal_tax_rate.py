...
class health_marginal_tax_rate(Variable):
    ...
    def formula(person, period, parameters):
        ...
        hh = person.household
        netinc_base = hh("household_net_income", period) + hh(
            "healthcare_benefit_value", period
        )
        ...
        for idx in range(1, 1 + adult_count):
            ...
            for var in sim.tax_benefit_system.variables:
                if (
                    var not in sim.input_variables
                    or var == "employment_income"
                ):
            ...
            netinc_alt = hh_alt("household_net_income", period) + hh_alt(
                "healthcare_benefit_value", period
            )
