    ...
    def formula(tax_unit, period, parameters):
        ...
        if p.amended_structure.applies:
            ...
            second_phase_out_start = (
                p.phase_out.amended_structure.phase_out.second_start[
                    filing_status
                ]
            )
            second_excess_agi = max_(0, agi - second_phase_out_start)
            second_phase_out_increment = (
                p.phase_out.amended_structure.phase_out.second_increment[
                    filing_status
                ]
            )
            ...
            second_percentage_reduction = (
                second_increments
                * p.phase_out.amended_structure.phase_out.rate
            )
            return max_(
                p.phase_out.amended_structure.phase_out.second_min,
