...
class taxable_unemployment_compensation(Variable):
    ...
    def formula(person, period, parameters):
        ...
        # total_unemployment_compensation, so that per-person AGI reflects the
        # actual recipient (required by states with combined-return optimization).
        tax_unit_taxable_uc = person.tax_unit(
            "tax_unit_taxable_unemployment_compensation", period
        )
        person_uc = person("total_unemployment_compensation", period)
