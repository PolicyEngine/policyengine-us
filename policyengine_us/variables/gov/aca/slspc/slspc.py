...
class second_lowest_silver_plan_cost(Variable):
    ...
    # 1. Inputs from user: state, county which determine -> rating area 
    # 2. Inputs from user: age -> initial calculation on a person level 
    # 
...
class person_aca_slspc(Variable):
    ...
    def formula(person, period, parameters):
        ...
        states_with_custom_curves = [
            "AL", "DC", "MA", "MN", "MS", "OR", "UT"
        ]
