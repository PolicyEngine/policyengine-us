...
class ca_state_medicaid_cost_if_enrolled(Variable):
    value_type = float
    entity = Person
    label = (
        "Per capita California state-funded Medicaid cost by eligibility group"
    )
    unit = USD
    ...
    def formula(person, period, parameters):
        ...
        # CA state Medicaid provides equivalent benefits
        state = person.household("state_code", period)

        # Determine group based on medicaid_category for CA state Medicaid
        # since medicaid_group requires federal eligibility
        cat = person("medicaid_category", period)
        cats = cat.possible_values

        # Map categories to groups similar to medicaid_group logic
        is_child = (
            (cat == cats.INFANT)
        ...
        is_expansion = cat == cats.ADULT

        # Assign appropriate group
        group = select(
            [is_disabled, is_child, is_non_expansion, is_expansion],
            [
