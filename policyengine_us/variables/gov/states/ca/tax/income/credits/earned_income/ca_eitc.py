...
class ca_eitc(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        second_phase_out_start_eitc = p.phase_out.final.start.calc(child_count) # Expressed as the EITC amount at which the second phase-out starts, not the income level.

        maximum_eitc = phase_in_max_income * phase_in_rate
        eitc_fall_by_first_phase_out = maximum_eitc - second_phase_out_start_eitc
        earnings_range_of_first_phase_out = eitc_fall_by_first_phase_out / phase_out_rate
        ...
        percentage_along_second_phase_out = min_(
            (earned_income - second_phase_out_start) / (second_phase_out_end - second_phase_out_start),
            1,
        )
