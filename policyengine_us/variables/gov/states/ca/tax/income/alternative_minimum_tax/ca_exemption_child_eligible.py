...
class ca_exemption_child_eligible(Variable):
    ...
    def formula(person, period, parameters):
        ...
        no_income = (person("age", period) == p.age_lower_threshold) & (person("earned_income", period) == 0)
        student_no_income = person("is_full_time_student", period) & (person("earned_income", period) == 0) & (person("age", period) < p.age_upper_threshold)
