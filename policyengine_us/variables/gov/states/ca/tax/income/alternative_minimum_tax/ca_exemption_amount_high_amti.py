...
class ca_exemption_amount_high_amti(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        over_threshold = tax_unit("ca_amti", period) >= p.exemption_amt_upper_threshold[filing_status]

        return where (eligible_child,
                    where(over_threshold, 0, min_(exemption_amount_high, exemption_amount_child_total)),
