...
class is_ar_sra_ess_eligible(Variable):
    value_type = bool
    entity = SPMUnit
    label = "Eligible for Arkansas SRA Extended Support Services (ESS) track"
    ...
    def formula(spm_unit, period, parameters):
        ...
        was_tea = spm_unit("was_tea_recipient", period.this_year)
        months_since = spm_unit("months_since_tea_exit", period)
        ...
        max_hours = spm_unit.max(hours)
        year_1_active = max_hours >= p.activity_hours_ess_year_1
        year_2_active = max_hours >= p.activity_hours_ess_year_2
