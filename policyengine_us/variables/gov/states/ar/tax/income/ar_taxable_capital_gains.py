...
class ar_taxable_capital_gains(Variable):
    ...
    def formula(person, period, parameters):
        ...
        net_cap_gain = max_(0, net_lt_capital_gains - net_st_capital_gains)
        p = parameters(
            period
        ).gov.states.ar.tax.income.gross_income.capital_gains
        capped_net_cap_gain = min_(net_cap_gain, p.cap)
        taxable_capped_net_cap_gain = capped_net_cap_gain * (1 - p.exempt_rate)
