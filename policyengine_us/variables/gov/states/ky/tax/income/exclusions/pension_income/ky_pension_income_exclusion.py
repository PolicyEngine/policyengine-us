...
class ky_pension_income_exclusion(Variable):
    ...
    def formula(person, period, parameters):
        ...
        exemption_percentage = person("ky_service_credits_percentage_pre_1998", period)

        # Get the exempt amount, based on eligibility and share of months worked pre-1998.
        exempt_amount = exemption_eligible * exemption_percentage * pension_income
        non_exempt_amount = pension_income - exempt_amount

        # Get retirement income not already exempt.
        other_income = (
            add(person, period, p.other_retirement_income_sources) + non_exempt_amount
        )
