...
class ma_tafdc_earned_income(Variable):
    ...
    reference = "https://www.masslegalservices.org/content/62-what-income-counted"
    defined_for = StateCode.MA

    def formula(tax_unit, period, parameters):
        p = parameters(period).gov.states.ma.dta.tafdc.gross_income
        total_earned_income = add(tax_unit, period, p.earned)
        reduced_income = (total_earned_income - p.earned_income_disregard.flat) * p.earned_income_disregard.percentage
        dependent_care_deduction = tax_unit.sum(tax_unit.members("ma_tafdc_dependent_care_deduction", period))
