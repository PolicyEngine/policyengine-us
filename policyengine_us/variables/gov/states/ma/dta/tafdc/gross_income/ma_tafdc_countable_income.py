...
class ma_tafdc_countable_income(Variable):
    ...
    def formula(person, period, parameters):
        p = parameters(period).gov.states.ma.dta.tafdc.gross_income.deductions.earned_income_disregard
        ...
        fully_disregard_eligible = spm_unit("ma_tafdc_eligible_fully_disregard", period)
        fully_disregard = (monthly_income - monthly_income * p.fully_disregard.percentage) * p.fully_disregard.applicable_months
        ...
        last_six_months = (
            monthly_income - flat_disregard
        ) * p.percentage * remaining_months 
        first_six_month = where(fully_disregard_eligible, fully_disregard, last_six_months)
        ...
        return max_(0, total_adjusted_income - dependent_care_deduction) * ~is_grandparent
