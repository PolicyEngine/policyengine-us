...
class oh_lump_sum_retirement_credit(Variable):
    ...
    def formula(tax_unit, period, parameters):
        p = parameters(
            period
        ).gov.states.oh.tax.income.credits
        ...
        head_divisor = p.lump_sum_retirement_multiple.calc(age_head)
        spouse_divisor = p.lump_sum_retirement_multiple.calc(age_spouse)

        head_pension = tax_unit.sum(is_head * pension_income)
        spouse_pension = tax_unit.sum(is_spouse * pension_income)

        head_credit = p.retirement_income.amount.calc(head_pension / head_divisor, right=True) * head_divisor
        spouse_credit = p.retirement_income.amount.calc(
            spouse_pension / spouse_divisor, right=True
        ) * spouse_divisor

        # distribution_received = tax_unit("c05700", period) > 0 # received lump sum distribution
        distribution_received = tax_unit("form_4972_lumpsum_distributions", period) > 0 
        
        return (head_credit + spouse_credit) if distribution_received else 0
