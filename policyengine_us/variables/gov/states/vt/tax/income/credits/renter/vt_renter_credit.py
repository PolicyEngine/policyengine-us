...
class vt_renter_credit(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        match_full_credit_income = p.full_credit_income_limits[family_size][county_str]
        match_partial_credit_income = p.partial_credit_income_limits[family_size][county_str]
        match_base_credit_amount = p.base_credit_amount[family_size][county_str]
        
        if vt_renter_credit_income < match_full_credit_income:
            if subsidized == 0:
                return match_base_credit_amount* p.share_rent_rate**share_rent
            else: 
            ...
            if subsidized == 0:
                return (match_partial_credit_income - match_full_credit_income)/(match_partial_credit_income - vt_renter_credit_income) * match_base_credit_amount * p.share_rent_rate**share_rent
            else: 
                return (match_partial_credit_income - match_full_credit_income)/(match_partial_credit_income - vt_renter_credit_income) * rent_pay*p.rent_rate* p.share_rent_rate**share_rent
