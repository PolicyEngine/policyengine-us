...
class vt_renter_credit(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        percent_reabte_claimable = (match_partial_credit_income - match_full_credit_income)/(match_partial_credit_income - vt_renter_credit_income)
        conditions = [
            vt_renter_credit_income > match_partial_credit_income,
            vt_renter_credit_income < match_full_credit_income & ~subsidized,
            vt_renter_credit_income < match_full_credit_income & subsidized,
            match_full_credit_income <= vt_renter_credit_income <= match_partial_credit_income & ~subsidized,
            match_full_credit_income <= vt_renter_credit_income <= match_partial_credit_income & subsidized,
        ]
        values = [
            0,
            ...
            percent_reabte_claimable* match_base_credit_amount* p.share_rent_rate**share_rent,
            percent_reabte_claimable* rent_pay* p.rent_rate* p.share_rent_rate**share_rent,
        ]
