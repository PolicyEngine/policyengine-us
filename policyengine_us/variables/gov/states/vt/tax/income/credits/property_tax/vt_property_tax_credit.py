...
class vt_property_tax_credit(Variable):
    value_type = float
    entity = TaxUnit
    label = "Vermont property tax credit"
    unit = USD
    definition_period = YEAR
    reference = (
        # "http://webserver.rilin.state.ri.us/Statutes/TITLE44/44-33/44-33-9.htm"
    )
    # defined_for = "vt_property_tax_credit_eligible"

    def formula(tax_unit, period, parameters):
        p = parameters(period).gov.states.ri.tax.income.credits.property_tax
        household_income = tax_unit("vt_property_tax_household_income", period)
