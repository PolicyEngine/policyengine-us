...
class vt_personal_exemptions(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        claimable_head = tax_unit("dsi", period)
        claimable_spouse = tax_unit("dsi_spouse", period)
        # If claimable elsewhere, it is not eligible for vt personal exemption (line 5a, line 5b)
        eligible_head = invert(claimable_head).astype(int)
        eligible_spouse = invert(claimable_spouse).astype(int)
