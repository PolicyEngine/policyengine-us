...
class wv_ccap(Variable):
    ...
    def formula(spm_unit, period, parameters):
        ...
        in_monthly_range = (
            monthly_care_days >= p.billing.monthly_rate_min_days
        ...
        special_needs_supplement = where(
            is_disabled, p.supplements.special_needs, 0
        )
        non_trad = person("wv_ccap_non_traditional_hours", period)
        non_trad_supplement = where(
            non_trad, p.supplements.non_traditional_hours, 0
        )
