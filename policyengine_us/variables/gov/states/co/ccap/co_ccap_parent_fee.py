...
class co_ccap_parent_fee(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        re_determination_eligible = tax_unit("co_ccap_re_determination", period)

        if (is_entry * entry_eligible) | (is_re_determination * re_determination_eligible):
            ...
            rate = p.parent_fee_rate_by_child_care_hours.calc(childcare_hours_per_week)
            non_discouted_fee = tax_unit.sum(child_age_eligible * (base_parent_fee + add_on_parent_fee) * rate)

            # Identify whether the filers are eligible for a discount.
            rating = tax_unit.sum(person("co_quality_rating_of_child_care_facility", period) >= 3)
            
            discounted_rate = p.parent_fee_quality_rating_rate

            return where(rating >= 1,  non_discouted_fee * discounted_rate, non_discouted_fee)
