...
class co_ccap_parent_fee(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        re_determination_eligible = tax_unit("co_ccap_re_determination_eligible", period)
        ...
        if (is_entry * entry_eligible) | (is_re_determination * re_determination_eligible):
            ...
            rate = p.parent_fee_rate_by_child_care_hours.calc(childcare_hours_per_day, right=True)
            ...
            discount_eligible = tax_unit.sum(p.is_quality_rating_discounted.calc(rating)) > 0
            
            discounted_rate = p.quality_discounted_rate

            return np.round(where(discount_eligible,  non_discouted_fee * discounted_rate, non_discouted_fee), 2)
