...
class md_two_income_subtraction(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        #print("\nMD -----------------------")
        #print("MD us_agi=", us_agi)
        gross_income = person("irs_gross_income", period)
        #print("MD gross_income=", gross_income)
        is_head = person("is_tax_unit_head", period) == 1
        is_spouse = person("is_tax_unit_spouse", period) == 1
        head_gross_income = tax_unit.sum(
            where(is_head, gross_income, 0)
        )
        #print("MD head_gross_income=", head_gross_income)
        couple_gross_income = tax_unit.sum(
            where(is_head | is_spouse, gross_income, 0)
        )
        #print("MD couple_gross_income=", couple_gross_income)
        head_frac = where(
            couple_gross_income > 0,
            head_gross_income / couple_gross_income,
            1
        )
        #print("MD head_frac=", head_frac)
        head_us_agi = head_frac * us_agi
        #print("MD head_us_agi=", head_us_agi)
        spouse_us_agi = (1 - head_frac) * us_agi
        #print("MD spouse_us_agi=", spouse_us_agi)
        ...
        for subtraction in p.sources:
            ...
            if subtraction == "md_dependent_care_subtraction":
                ...
                head_frac = where(
                    tax_unit("filing_status", period) == "Joint",
                    0.5,
                    1.0
                )
        ...
        #print("MD two-income subtraction=", capped_min_agi_add_sub)
