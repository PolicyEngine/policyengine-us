...
class la_school_readiness_credit(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        p = parameters(period).gov.states.la.tax.income.credits.school_readiness
        # determine school readiness credit amount
        child_care_credit = add(
            tax_unit, period, ["la_refundable_cdcc", "la_non_refundable_cdcc"]
        )
        eligible_child = person("la_school_readiness_credit_eligible_child", period)
        quality_rating = person("la_quality_rating_of_child_care_facility", period)
