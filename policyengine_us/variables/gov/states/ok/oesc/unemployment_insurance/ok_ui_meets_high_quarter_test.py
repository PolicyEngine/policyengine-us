...
class ok_ui_meets_high_quarter_test(Variable):
    ...
    Prong 1 (§ 2-207(A)(1)) requires base-period taxable wages of at least the
    statutory minimum. Prong 2 (§ 2-207(A)(2)) requires total base-period
    wages of at least 1.5x the claimant's total high-quarter wages; the statute
    uses total (uncapped) wages, not the taxable-capped figure.
    """
    ...
    def formula(person, period, parameters):
        ...
        high_quarter_total_wages = person("ok_ui_high_quarter_total_wages", period)
        meets_taxable_minimum = base_period_taxable_wages >= p.min_taxable_wages
        meets_multiplier = base_period_total_wages >= (
            p.min_total_wages_multiplier * high_quarter_total_wages
        )
