...
class ny_ctc_pre_reduction_base_credit(Variable):
    ...
    documentation = (
        "New York's Empire State Child Credit Worksheet A / B Part 1"
    )
    unit = USD
    ...
    def formula(tax_unit, period, parameters):
        ...
        claimed_additional_credits = tax_unit(
            "ny_ctc_claimed_additional_credits", period
        )
        total_exclusions = add(
            tax_unit,
            period,
            ["foreign_earned_income_exclusion", "puerto_rico_income"],
        )
        agi_with_exclusion_amount = where(
