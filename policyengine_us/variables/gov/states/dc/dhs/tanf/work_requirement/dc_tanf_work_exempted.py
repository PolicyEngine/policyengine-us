...
class dc_tanf_work_exempted(Variable):
    ...
    def formula(person, period, parameters):
        ...
        # Old person can exempted
        elderly = age > p.elderly_age_threshold
        # Pregnant person can exempted
        is_pregnant = person("is_pregnant", period)
        # Eligible child can exempted
        eligible_child = person("dc_pap_eligible_child", period)
        # Single parent can exempted with infant less than 1 years old can exempted
        filing_status = person.tax_unit("filing_status", period)
        joint = filing_status == filing_status.possible_values.JOINT
        has_infant = person.spm_unit.any(age < p.infant_age_threshold)
        is_head = person("is_tax_unit_head", period)
        single_parent_exempted = ~joint & has_infant & is_head
