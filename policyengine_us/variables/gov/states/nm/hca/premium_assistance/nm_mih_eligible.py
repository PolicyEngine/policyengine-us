...
class nm_mih_eligible(Variable):
    value_type = bool
    entity = TaxUnit
    label = (
    ...
    documentation = (
        "A tax unit is eligible for the Middle Income Household component when "
        "that component is in effect, at least one member is enrolled on the "
        "Marketplace, the tax unit does not file separately, and household "
        "income is above the federal poverty line limit. This reuses the "
        "non-income pieces of federal ACA PTC eligibility (Marketplace "
        "enrollment and the married-filing-separately exclusion) but not the "
        "income test, which fails above 400% FPL."
    )
    ...
    def formula(tax_unit, period, parameters):
        ...
        return in_effect & pays_premium & not_separate & income_eligible
