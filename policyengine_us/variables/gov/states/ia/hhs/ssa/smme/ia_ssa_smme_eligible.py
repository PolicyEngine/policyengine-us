...
class ia_ssa_smme_eligible(Variable):
    ...
    def formula(person, period, parameters):
        ...
        # joint-claim couple's) combined income, not on a larger SPM unit,
        # so size the FPL by the SSI claim — 1 person for an individual,
        # 2 for a joint claim — rather than by SPM unit size. For joint
        # claims, ssi_countable_income returns the per-spouse half of the
        # combined marital countable, so aggregate across the marital unit
        # before comparing to the 2-person FPL.
        ...
        # YEAR-defined FLOW vars auto-divide to monthly at MONTH period.
        own_monthly = person("ssi_countable_income", period)
        marital_monthly = person.marital_unit.sum(own_monthly)
        countable_monthly = where(joint_claim, marital_monthly, own_monthly)
        ...
        # FPG parameters are annual values (no auto-divide for parameters).
