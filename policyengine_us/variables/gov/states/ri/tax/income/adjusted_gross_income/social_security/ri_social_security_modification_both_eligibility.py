...
class ri_social_security_modification_both_eligibility(Variable):
    value_type = float
    entity = TaxUnit
    label = "Rhode Island Social Security Modification Eligibility for Head and Spouse"
    unit = USD
    ...
    def formula(tax_unit, period, parameters):
        ...
        head_eligible = tax_unit.max(age_conditions & is_head)

        spouse_eligible = tax_unit.max(age_conditions & is_spouse)

        age_is_eligible = head_eligible + spouse_eligible >= 2.0
