...
class hi_ccap_maximum_monthly_rate(Variable):
    ...
    def formula(person, period, parameters):
        ...
        age_group = person("hi_ccap_age_group", period)
        hours_tier = person("hi_ccap_hours_tier", period)
        rate = p.rates[provider_category][age_group][hours_tier]
        # Exhibit I's part-time band starts at 1 monthly hour, so a child with
        # no authorized care hours (no care need) receives no payment.
