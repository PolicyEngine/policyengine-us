...
class ga_retirement_exclusion(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        age_younger = (person("age", period) >= p.age.younger) & (
            person("age", period) < p.age.older
        )
        age_older = person("age", period) >= p.age.older
        cap_younger = p.cap.younger
        cap_older = p.cap.older
        ...
        below_age_younger = person("age", period) < p.age.younger
        disabled = person("is_disabled", period)
        disabled_eligible = (head_or_spuse) & below_age_younger & disabled
