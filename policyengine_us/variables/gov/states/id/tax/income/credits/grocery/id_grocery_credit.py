...
class id_grocery_credit(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        eligible_fraction = qualified_months / MONTHS_IN_YEAR
        base = person("id_grocery_credit_base", period) * eligible_fraction
        aged = (
            tax_unit.members("id_grocery_credit_aged", period)
            * eligible_fraction
        )
        return tax_unit.sum(base + aged)
