...
class tanf_if_takes_up(Variable):
    value_type = float
    entity = SPMUnit
    definition_period = YEAR
    label = "TANF if takes up"
    documentation = (
        "Value of Temporary Assistance for Needy Families benefit the SPM "
        "unit would receive if it takes up the program, summing all "
        "state-specific TANF programs, regardless of reported take-up."
    )
    unit = USD
    ...
    def formula(spm_unit, period, parameters):
        if parameters(period).gov.hhs.tanf.abolish_tanf:
            return 0
        return add(spm_unit, period, STATE_TANF_VARIABLES)
