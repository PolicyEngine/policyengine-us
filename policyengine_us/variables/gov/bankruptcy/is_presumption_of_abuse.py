...
class is_presumption_of_abuse(Variable):
    ...
    reference = (
        "https://www.cacb.uscourts.gov/sites/cacb/files/documents/forms/122A2.pdf#page=8"
    )
    documentation = "Line 39-xx in form 122A-2"


    def formula(spm_unit, period, parameters):
        adjusted_monthly_income = spm_unit("chapter_7_bankruptcy_adjust_monthly_income", period)
        total_deductions = spm_unit("chapter_7_bankruptcy_total_deductions", period)
        monthly_disposable_income = adjusted_monthly_income - total_deductions
        monthly_disposable_income_60_months = monthly_disposable_income * MONTHS_IN_YEAR * 5
