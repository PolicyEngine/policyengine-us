...
class snap_excess_shelter_expense_deduction(Variable):
    ...
    def formula(spm_unit, period, parameters):
        ...
        # Standard utility allowances are not expenses paid by or billed
        # to an ineligible member, so only actual housing costs are
        # prorated (7 CFR 273.11(c)(2)(iii)).
        utility_allowance = spm_unit("snap_utility_allowance", period)
        housing_cost = (
            expense_share * add(spm_unit, period, ["housing_cost"]) + utility_allowance
        )
