...
class lifeline(Variable):
    ...
    def formula(spm_unit, period, parameters):
        ...
        if p_ca.in_effect:
            ca_amount = p_ca.amount
            in_ca = state_code == "CA"
            amount = where(in_ca, ca_amount, amount)

        p_or = parameters(period).gov.states["or"].fcc.lifeline
        if p_or.in_effect:
            or_amount = p_or.amount
            in_or = state_code == "OR"
            amount = where(in_or, or_amount, amount)

        max_amount = (
            amount + is_rural_tribal * amounts.rural_tribal_supplement
        ) * MONTHS_IN_YEAR
