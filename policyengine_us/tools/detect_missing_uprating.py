...
def check_time_series(data, current_year=2025, debug_file=None, min_growth_rate=None, max_growth_rate=None):
    """
    ...
    if min_growth_rate is None or max_growth_rate is None:
        from policyengine_us.tools.uprating_config import MIN_GROWTH_RATE, MAX_GROWTH_RATE
    ...
    has_uprating = 'uprating' in data or 'metadata' in data and isinstance(data.get('metadata'), dict) and 'uprating' in data.get('metadata', {})
    ...
    if debug_file and date_keys_found > 0:
        print(f"DEBUG check_time_series: found {date_keys_found} date keys, {len(year_keys)} valid years")
    ...
    if len(year_keys) >= 3:
        ...
        for i in range(len(year_keys) - 2):
            if year_keys[i+1] == year_keys[i] + 1 and year_keys[i+2] == year_keys[i] + 2:
                ...
                if len(vals) >= 3 and len(set(str(v) for v in vals)) > 1:  # At least 2 different values
                    ...
                    try:
                        ...
                        if growth_rates:
                            ...
                            all_within_range = all(min_growth_rate <= gr <= max_growth_rate for gr in growth_rates)

                            if all_within_range:
                                return True, year_keys, has_uprating, avg_growth
...
def scan_parameter_file(file_path, current_year=2025, verbose=False):
    ...
    def recurse(obj, path="", parent_has_uprating=False):
        ...
        has_pattern, years, has_uprating, avg_growth = check_time_series(obj, current_year, debug_file=debug)

        if debug and has_pattern:
            print(f"DEBUG: {path or 'ROOT'} - pattern={has_pattern}, years={len(years)}, has_uprating={has_uprating}, parent={parent_has_uprating}, avg_growth={avg_growth:.2%}" if avg_growth else f"DEBUG: {path or 'ROOT'} - pattern={has_pattern}, years={len(years)}, has_uprating={has_uprating}, parent={parent_has_uprating}")
        ...
        if has_pattern and not has_any_uprating:
            ...
            if verbose:
                growth_str = f" (avg growth: {avg_growth:.2%})" if avg_growth else ""
                print(f"Found: {file_path} @ {path} - {len(years)} years{growth_str}")

        # Recurse into nested structures
        for key, value in obj.items():
            # Skip metadata keys
            if key in ['metadata', 'uprating', 'description', 'unit', 'reference']:
                continue
...
def main(verbose=False):
    ...
    parameters_dir = Path("/Users/pavelmakarchuk/policyengine-us/policyengine_us/parameters")

    if not parameters_dir.exists():
        print(f"Error: Parameters directory not found at {parameters_dir}")
        return

    print("Scanning for parameters with annual updates but missing uprating metadata...\n")

    findings_by_file = detect_missing_uprating(parameters_dir, verbose=verbose)

    # Report findings
    if not findings_by_file:
        print("✓ No issues found - all annually-updated parameters have uprating metadata!")
        return

    # Sort files by max years found in any parameter
    sorted_files = sorted(findings_by_file.items(),
    ...
    print(f"Found {total_params} parameters in {total_files} files that likely need uprating metadata:\n")
    ...
    for rel_path, file_findings in sorted_files:
        ...
        growth_rates = [f.get('avg_growth') for f in file_findings if f.get('avg_growth') is not None]
        avg_growth = sum(growth_rates) / len(growth_rates) if growth_rates else None

        years_str = f"{min_year}-{max_year} ({max_years} years)"
        growth_str = f", avg growth: {avg_growth:.2%}/year" if avg_growth else ""

        print(f"  • {rel_path}")
        print(f"    Years: {years_str}{growth_str}")
        if len(file_findings) > 1:
            print(f"    Parameters: {len(file_findings)} parameters in this file")
        print()
