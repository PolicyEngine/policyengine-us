...
def EI_PayrollTax(
    ...
    txearn_sey_p = min(max(0.0, sey_p * sey_frac), SS_Earnings_c - txearn_was_p)
    txearn_sey_s = min(max(0.0, sey_s * sey_frac), SS_Earnings_c - txearn_was_s)
    ...
    setax_mc_p = FICA_mc_trt * max(0.0, sey_p * sey_frac)
    setax_mc_s = FICA_mc_trt * max(0.0, sey_s * sey_frac)
    ...
    was_plus_sey_p = gross_was_p + max(0.0, sey_p * sey_frac)
    was_plus_sey_s = gross_was_s + max(0.0, sey_s * sey_frac)
    extra_ss_income_p = max(0.0, was_plus_sey_p - SS_Earnings_thd)
    extra_ss_income_s = max(0.0, was_plus_sey_s - SS_Earnings_thd)
    extra_payrolltax = extra_ss_income_p * FICA_ss_trt + extra_ss_income_s * FICA_ss_trt
    ...
    ptax_oasdi = (
        ptax_ss_was_p + ptax_ss_was_s + setax_ss_p + setax_ss_s + extra_payrolltax
    )
    ...
    c03260 = (1.0 - ALD_SelfEmploymentTax_hc) * 0.5 * setax
    earned = max(0.0, e00200p + e00200s + sey - c03260)
    earned_p = max(
        0.0,
...
def DependentCare(
    ...
    if earned <= ALD_Dependents_thd[MARS - 1]:
        care_deduction = ((1.0 - ALD_Dependents_hc) * nu13 * ALD_Dependents_Child_c) + (
            (1.0 - ALD_Dependents_hc) * elderly_dependents * ALD_Dependents_Elder_c
        )
    else:
        care_deduction = 0.0
...
def Adj(
    ...
    c02900 = (
        (1.0 - ALD_StudentLoan_hc) * e03210
        + c03260
        + (1.0 - ALD_EarlyWithdraw_hc) * e03400
        + (1.0 - ALD_AlimonyPaid_hc) * e03500
        + (1.0 - ALD_AlimonyReceived_hc) * e00800
        + (1.0 - ALD_EducatorExpenses_hc) * e03220
        + (1.0 - ALD_Tuition_hc) * e03230
        + (1.0 - ALD_DomesticProduction_hc) * e03240
        + (1.0 - ALD_HSADeduction_hc) * e03290
        + (1.0 - ALD_SelfEmp_HealthIns_hc) * e03270
...
def ALD_InvInc_ec_base(
    p22250, p23250, sep, e00300, e00600, e01100, e01200, invinc_ec_base
):
    """
    ...
    cgain = max((-3000.0 / sep), p22250 + p23250)
...
def CapGains(
    ...
    # limitation on capital losses
    c01000 = max((-3000.0 / sep), c23650)
    # compute total investment income
    invinc = e00300 + e00600 + c01000 + e01100 + e01200
    # compute exclusion of investment income from AGI
    invinc_agi_ec = ALD_InvInc_ec_rt * max(0.0, invinc_ec_base)
    ...
    ymod3 = (1.0 - ALD_StudentLoan_hc) * e03210 + e03230 + e03240
...
def SSBenefits(
    ...
    if ymod < SS_thd50[MARS - 1]:
        ...
        c02500 = min(
            SS_percentage2 * (ymod - SS_thd85[MARS - 1])
            + SS_percentage1 * min(e02400, SS_thd85[MARS - 1] - SS_thd50[MARS - 1]),
...
def AGI(
    ...
    if exact == 1:  # exact calculation as on tax forms
        line5 = max(0.0, c00100 - II_em_ps[MARS - 1])
        line6 = math.ceil(line5 / (2500.0 / sep))
        line7 = II_prt * line6
        c04600 = max(0.0, pre_c04600 * (1.0 - line7))
    else:  # smoothed calculation needed for sensible mtr calculation
        dispc_numer = II_prt * (c00100 - II_em_ps[MARS - 1])
        dispc_denom = 2500.0 / sep
        dispc = min(1.0, max(0.0, dispc_numer / dispc_denom))
        c04600 = pre_c04600 * (1.0 - dispc)
...
def ItemDedCap(
    ...
    cap = max(0.0, ID_AmountCap_rt * c00100)
    ...
    overage = max(0.0, gross_ded_amt - cap)
...
def ItemDed(
    ...
    c17000 = max(0.0, e17500_capped - c17750) * (1.0 - ID_Medical_hc)
    ...
    c18500 = min((1.0 - ID_RealEstate_hc) * e18500_capped, ID_RealEstate_c[MARS - 1])
    ...
    c18300 = (c18400 + c18500) * (1.0 - ID_AllTaxes_hc)
    c18300 = min(c18300, ID_AllTaxes_c[MARS - 1])
    # Interest paid
    c19200 = e19200_capped * (1.0 - ID_InterestPaid_hc)
    ...
    c19700 = max(0.0, c19700 - charity_floor) * (1.0 - ID_Charity_hc)
    c19700 = min(c19700, ID_Charity_c[MARS - 1])
    # Casualty
    c20500 = max(0.0, g20500_capped - ID_Casualty_frt * posagi) * (1.0 - ID_Casualty_hc)
    c20500 = min(c20500, ID_Casualty_c[MARS - 1])
    # Miscellaneous
    c20400 = e20400_capped
    c20750 = ID_Miscellaneous_frt * posagi
    c20800 = max(0.0, c20400 - c20750) * (1.0 - ID_Miscellaneous_hc)
    ...
    if c21060 > nonlimited and c00100 > limitstart:
        ...
        dedpho = ID_prt * max(0.0, posagi - limitstart)
...
def AdditionalMedicareTax(
    ...
    payrolltax: float
        payroll tax augmented by Additional Medicare Tax
    """
    line8 = max(0.0, sey) * (1.0 - 0.5 * (FICA_mc_trt + FICA_ss_trt))
...
def StdDed(
    ...
    if DSI == 1:
        c15100 = max(350.0 + earned, STD_Dep)
...
def TaxInc(
    ...
    pre_qbid_taxinc = max(0.0, c00100 - max(c04470, standard) - c04600 - ui_excluded)
    # calculate qualified business income deduction
    qbided = 0.0
    qbinc = max(0.0, e00900 + e26270 + e02100 + e27200)
    if qbinc > 0.0 and PT_qbid_rt > 0.0:
        ...
        taxinc_cap = PT_qbid_rt * max(0.0, pre_qbid_taxinc - net_cg)
        qbided = min(qbided, taxinc_cap)

        # apply qbid phaseout
        if qbided > 0.0 and pre_qbid_taxinc > PT_qbid_ps[MARS - 1]:
            excess = pre_qbid_taxinc - PT_qbid_ps[MARS - 1]
            qbided = max(0.0, qbided - PT_qbid_prt * excess)

    # calculate taxable income after qualified business income deduction
    c04800 = max(0.0, pre_qbid_taxinc - qbided)
...
def SchXYZ(
    ...
    pt_taxinc = max(0.0, pt_passive + pt_active)
...
def GainsTax(
    ...
    if c01000 > 0.0 or c23650 > 0.0 or p23250 > 0.0 or e01100 > 0.0 or e00650 > 0.0:
    ...
    if hasqdivltcg == 1:
        ...
        dwks2 = e00650
        dwks3 = e58990
        dwks4 = 0.0  # always assumed to be zero
        dwks5 = max(0.0, dwks3 - dwks4)
        dwks6 = max(0.0, dwks2 - dwks5)
        ...
        if e01100 > 0.0:
            ...
            c24510 = max(0.0, dwks7) + e01100
        dwks9 = max(0.0, c24510 - min(0.0, e58990))
        ...
        dwks22 = dwks20
        dwks23 = max(0.0, dwks21 - dwks22)
        dwks25 = min(CG_brk2[MARS - 1], dwks1)
        dwks26 = dwks19 + dwks20
        dwks27 = max(0.0, dwks25 - dwks26)
        ...
        hi_base = max(0.0, cg_all - CG_brk3[MARS - 1])
        ...
        dwks36 = max(0.0, dwks34 - dwks1)
        dwks37 = max(0.0, dwks33 - dwks36)
        dwks38 = 0.25 * dwks37
        # break in worksheet lines
    ...
    c05700 = 0.0  # no Form 4972, Lump Sum Distributions
    taxbc = c05700 + c05100
...
def AGIsurtax(c00100, MARS, AGI_surtax_trt, AGI_surtax_thd, taxbc, surtax):
    ...
    surtax: float
        Surtax on AGI above some threshold
    """
    if AGI_surtax_trt > 0.0:
        hiAGItax = AGI_surtax_trt * max(c00100 - AGI_surtax_thd[MARS - 1], 0.0)
        taxbc += hiAGItax
        surtax += hiAGItax
    return (taxbc, surtax)
...
def AMT(
    ...
    if standard == 0.0:
        c62100 = (
            ...
            - e00700
            - c04470
            + max(0.0, min(c17000, 0.025 * c00100))
            + c18300
            + c20800
            - c21040
        )
    ...
    if MARS == 3:
        amtsepadd = max(0.0, min(AMT_em[MARS - 1], AMT_prt * (c62100 - AMT_em_pe)))
    else:
        amtsepadd = 0.0
    ...
    line3163 = AMT_rt1 * line30 + AMT_rt2 * max(0.0, (line30 - (AMT_brk1 / sep)))
    if dwks10 > 0.0 or dwks13 > 0.0 or dwks14 > 0.0 or dwks19 > 0.0 or e24515 > 0.0:
        ...
        line42 = AMT_rt1 * line41 + AMT_rt2 * max(0.0, (line41 - (AMT_brk1 / sep)))
        line44 = dwks14
        ...
        line53 = max(0.0, AMT_CG_brk2[MARS - 1] - line52)
        ...
        if line41 == line56:
            line57 = 0.0  # line57 is amount taxed at AMT_CG_rt3
            linex2 = 0.0  # linex2 is amount taxed at AMT_CG_rt4
        else:
            line57 = line46 - line56
            linex1 = min(line48, max(0.0, AMT_CG_brk3[MARS - 1] - line44 - line45))
        ...
        if line38 == 0.0:
            ...
            line61 = 0.25 * max(0.0, line30 - line41 - line56 - line57 - linex2)
    ...
    c09600 = max(0.0, line33 - max(0.0, taxbc - e07300 - c05700))
    c05800 = taxbc + c09600
...
def NetInvIncTax(
    ...
    if not NIIT_PT_taxed:
        NII = max(0.0, e00300 + e00600 + c01000 + e02000 - e26270)
    else:  # do not subtract e26270 from e02000
        NII = max(0.0, e00300 + e00600 + c01000 + e02000)
    niit = NIIT_rt * min(NII, max(0.0, modAGI - NIIT_thd[MARS - 1]))
    return niit
...
def F2441(
    ...
    c32800 = max(0.0, min(e32800, max_credit))
    ...
    if MARS == 2:
        ...
        c32890 = earned_p
    c33000 = max(0.0, min(c32800, min(c32880, c32890)))
    # credit is limited by AGI-related fraction
    if exact == 1:  # exact calculation as on tax forms
        # first phase-down from 35 to 20 percent
        tratio1 = math.ceil(max(((c00100 - CDCC_ps) * CDCC_prt), 0.0))
        crate = max(CDCC_frt, CDCC_crt - min(CDCC_crt - CDCC_frt, tratio1))
        # second phase-down from 20 percent to zero
        if c00100 > CDCC_ps2:
            tratio2 = math.ceil(max(((c00100 - CDCC_ps2) * CDCC_prt), 0.0))
            crate = max(0.0, CDCC_frt - min(CDCC_frt, tratio2))
    else:
        crate = max(CDCC_frt, CDCC_crt - max(((c00100 - CDCC_ps) * CDCC_prt), 0.0))
        if c00100 > CDCC_ps2:
            crate = max(0.0, CDCC_frt - max(((c00100 - CDCC_ps2) * CDCC_prt), 0.0))
    ...
    if CDCC_refundable:
        ...
        c07180 = min(max(0.0, c05800 - e07300), c33200)
        CDCC_refund = 0.0
...
def EITCamount(
    ...
    """
    eitc = min(
        (basic_frac * max_amount + (1.0 - basic_frac) * phasein_rate * earnings),
        max_amount,
    )
...
def EITC(
    ...
    if c59660 > 0.0:
        invinc = (
            e00400 + e00300 + e00600 + max(0.0, c01000) + max(0.0, (e02000 - e26270))
        )
        if invinc > EITC_InvestIncome_c:
            eitc = c59660 - EITC_excess_InvestIncome_rt * (invinc - EITC_InvestIncome_c)
            c59660 = max(0.0, eitc)
    return c59660


def RefundablePayrollTaxCredit(
    was_plus_sey_p, was_plus_sey_s, RPTC_c, RPTC_rt, rptc_p, rptc_s, rptc
):
    """
...
def ChildDepTaxCredit(
    ...
    if line3 > 0.0 and modAGI > CTC_ps[MARS - 1]:
        excess = modAGI - CTC_ps[MARS - 1]
        if exact == 1:  # exact calculation as on tax forms
            excess = 1000.0 * math.ceil(excess / 1000.0)
        line10 = max(0.0, line3 - CTC_prt * excess)
    else:
        line10 = line3
    if line10 > 0.0:
        # Worksheet Part 2
        line11 = c05800
        line12 = (
            e07260 * (1.0 - CR_ResidentialEnergy_hc)
            + e07300 * (1.0 - CR_ForeignTax_hc)
...
def PersonalTaxCredit(
    ...
    if c00100 < RRC_ps[MARS - 1]:
        ...
        prt = (c00100 - RRC_ps[MARS - 1]) / (RRC_pe[MARS - 1] - RRC_ps[MARS - 1])
...
def AmOppCreditParts(
    ...
    """
    if e87521 > 0.0:
        c87658 = max(0.0, 90000.0 * num - c00100)
        c87660 = 10000.0 * num
        if exact == 1:  # exact calculation as on tax forms
            c87662 = 1000.0 * min(1.0, round(c87658 / c87660, 3))
        else:
            c87662 = 1000.0 * min(1.0, c87658 / c87660)
        c87664 = c87662 * e87521 / 1000.0
        c10960 = 0.4 * c87664 * (1.0 - CR_AmOppRefundable_hc)
        c87668 = c87664 - c10960 * (1.0 - CR_AmOppNonRefundable_hc)
    else:
        c10960 = 0.0
        c87668 = 0.0
...
def SchR(
    ...
    if age_head >= 65 or (MARS == 2 and age_spouse >= 65):
        ...
        schr13b = max(0.0, e01500 - e01700)
        schr13c = schr13a + schr13b
        schr16 = max(0.0, c00100 - schr15)
        schr17 = 0.5 * schr16
        schr18 = schr13c + schr17
        schr19 = max(0.0, schr12 - schr18)
        schr20 = 0.15 * schr19
        schr21 = max(0.0, (c05800 - e07300 - c07180))
        c07200 = min(schr20, schr21) * (1.0 - CR_SchR_hc)
    else:  # if not calculating Schedule R credit
        c07200 = 0.0
    return c07200


def EducationTaxCredit(
    exact,
    e87530,
    MARS,
    c00100,
    ...
    if MARS == 2:
        c87570 = ETC_pe_Married * 1000.0
    else:
        c87570 = ETC_pe_Single * 1000.0
    c87590 = max(0.0, c87570 - c00100)
    c87600 = 10000.0 * num
    if exact == 1:  # exact calculation as on tax forms
        c87610 = min(1.0, round(c87590 / c87600, 3))
    else:
        c87610 = min(1.0, c87590 / c87600)
    c87620 = c87560 * c87610
    xline4 = max(0.0, c05800 - (e07300 + c07180 + c07200))
    xline5 = min(c87620, xline4)
    xline9 = max(0.0, c05800 - (e07300 + c07180 + c07200 + xline5))
    xline10 = min(c87668, xline9)
    c87680 = xline5 + xline10
    c07230 = c87680 * (1.0 - CR_Education_hc)
    return c07230
...
def NonrefundableCredits(
    ...
    avail = c05800
    # Foreign tax credit - Form 1116
    c07300 = min(e07300 * (1.0 - CR_ForeignTax_hc), avail)
    avail = avail - c07300
    ...
    avail = avail - c07230
    # Retirement savings credit - Form 8880
    c07240 = min(e07240 * (1.0 - CR_RetirementSavings_hc), avail)
    avail = avail - c07240
    # Child tax credit
    ...
    avail = avail - odc
    # Residential energy credit - Form 5695
    c07260 = min(e07260 * (1.0 - CR_ResidentialEnergy_hc), avail)
    avail = avail - c07260
    # General business credit - Form 3800
    c07400 = min(e07400 * (1.0 - CR_GeneralBusiness_hc), avail)
    avail = avail - c07400
    # Prior year minimum tax credit - Form 8801
    c07600 = min(e07600 * (1.0 - CR_MinimumTax_hc), avail)
    avail = avail - c07600
    # Schedule R credit
    c07200 = min(c07200, avail)
    avail = avail - c07200
    # Other credits
    c08000 = min(p08000 * (1.0 - CR_OtherCredits_hc), avail)
    avail = avail - c08000
...
def AdditionalCTC(
    ...
    c11070 = 0.0  # line15
    if line3 > 0.0 and line4 > 0.0:
        line5 = min(line3, line4)
        line7 = max(0.0, earned - ACTC_Income_thd)
...
def CTC_new(
    ...
    if childnum > 0:
        ...
        if posagi > ymax:
            ctc_new_reduced = max(0.0, ctc_new - CTC_new_prt * (posagi - ymax))
            ctc_new = min(ctc_new, ctc_new_reduced)
        if ctc_new > 0.0 and CTC_new_refund_limited:
            refund_new = max(0.0, ctc_new - c09200)
            ...
            limited_new = max(0.0, refund_new - limit_new)
            ctc_new = max(0.0, ctc_new - limited_new)
    else:
        ctc_new = 0.0
    return ctc_new
...
def Taxes(
    ...
    """
    if tbrk_base > 0.0:
        brk1 = max(tbrk1[MARS - 1] - tbrk_base, 0.0)
        brk2 = max(tbrk2[MARS - 1] - tbrk_base, 0.0)
        brk3 = max(tbrk3[MARS - 1] - tbrk_base, 0.0)
        brk4 = max(tbrk4[MARS - 1] - tbrk_base, 0.0)
        brk5 = max(tbrk5[MARS - 1] - tbrk_base, 0.0)
        brk6 = max(tbrk6[MARS - 1] - tbrk_base, 0.0)
        brk7 = max(tbrk7[MARS - 1] - tbrk_base, 0.0)
    else:
        brk1 = tbrk1[MARS - 1]
        brk2 = tbrk2[MARS - 1]
        brk3 = tbrk3[MARS - 1]
        ...
        brk7 = tbrk7[MARS - 1]
    return (
        rate1 * min(income, brk1)
        + rate2 * min(brk2 - brk1, max(0.0, income - brk1))
        + rate3 * min(brk3 - brk2, max(0.0, income - brk2))
        + rate4 * min(brk4 - brk3, max(0.0, income - brk3))
        + rate5 * min(brk5 - brk4, max(0.0, income - brk4))
...
def ComputeBenefit(calc, ID_switch):
    ...
    if ID_switch[0]:
        no_ID_calc.policy_param("ID_Medical_hc", [1.0])
    if ID_switch[1]:
        no_ID_calc.policy_param("ID_StateLocalTax_hc", [1.0])
    if ID_switch[2]:
        no_ID_calc.policy_param("ID_RealEstate_hc", [1.0])
    if ID_switch[3]:
        no_ID_calc.policy_param("ID_Casualty_hc", [1.0])
    if ID_switch[4]:
        no_ID_calc.policy_param("ID_Miscellaneous_hc", [1.0])
    if ID_switch[5]:
        no_ID_calc.policy_param("ID_InterestPaid_hc", [1.0])
    if ID_switch[6]:
        no_ID_calc.policy_param("ID_Charity_hc", [1.0])
    no_ID_calc._calc_one_year()  # pylint: disable=protected-access
    diff_iitax = no_ID_calc.array("iitax") - calc.array("iitax")
    benefit = np.where(diff_iitax > 0.0, diff_iitax, 0.0)
    return benefit
...
def BenefitSurtax(calc):
    ...
    if calc.policy_param("ID_BenefitSurtax_crt") != 1.0:
        ben = ComputeBenefit(calc, calc.policy_param("ID_BenefitSurtax_Switch"))
...
def BenefitLimitation(calc):
    ...
    if calc.policy_param("ID_BenefitCap_rt") != 1.0:
        benefit = ComputeBenefit(calc, calc.policy_param("ID_BenefitCap_Switch"))
        ...
        if calc.policy_param("ID_BenefitCap_Switch")[1]:  # statelocal
            ...
            deduct_exps += one_minus_hc * np.maximum(calc.array("e18400_capped"), 0.0)
...
def FairShareTax(
    ...
    surtax: float
        Individual income tax subtotal augmented by fstax
    """
    if FST_AGI_trt > 0.0 and c00100 >= FST_AGI_thd_lo[MARS - 1]:
        employee_share = 0.5 * ptax_was + 0.5 * setax + ptax_amc
        fstax = max(c00100 * FST_AGI_trt - iitax - employee_share, 0.0)
        thd_gap = max(FST_AGI_thd_hi[MARS - 1] - FST_AGI_thd_lo[MARS - 1], 0.0)
        if thd_gap > 0.0 and c00100 < FST_AGI_thd_hi[MARS - 1]:
...
CALCFUNCTIONS = {fn.__name__: fn for fn in CALCFUNCTIONS_LIST}
