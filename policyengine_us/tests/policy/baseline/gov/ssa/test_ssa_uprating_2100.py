"""Test that SSA uprating factors extend through 2100."""


def test_ssa_uprating_extends_to_2100():
    """Test that SSA uprating factors extend through 2100."""
    ...
    for year in test_years:
        value = parameters.gov.ssa.uprating(f"{year}-01-01")
        assert value > 0, f"No positive SSA uprating value for year {year}"
        values.append(value)
    ...
    for i in range(1, len(values)):
        assert (
            values[i] > values[i - 1]
        ), f"SSA uprating should increase over time"

    # Test that growth is consistent in the extended period
    year1, year2, year3 = 2040, 2041, 2042
    val1 = parameters.gov.ssa.uprating(f"{year1}-01-01")
    val2 = parameters.gov.ssa.uprating(f"{year2}-01-01")
    val3 = parameters.gov.ssa.uprating(f"{year3}-01-01")
