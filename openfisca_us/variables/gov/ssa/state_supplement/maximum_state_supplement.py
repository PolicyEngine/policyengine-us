...
class maximum_state_supplement(Variable):
    ...
    def formula(person, period, parameters):
        ...
        num_eligible_str = num_eligible.astype(int).astype(str)
        per_person_amount = max_(
            amounts[np.array([ssi_categories.AGED] * num_persons)][num_eligible_str] * is_aged,
            amounts[np.array([ssi_categories.BLIND] * num_persons)][num_eligible_str] * is_blind,
            amounts[np.array([ssi_categories.DISABLED] * num_persons)][num_eligible_str] * is_disabled,
        ) * MONTHS_IN_YEAR
