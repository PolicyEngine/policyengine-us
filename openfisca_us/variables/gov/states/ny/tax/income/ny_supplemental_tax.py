...
class ny_supplemental_tax(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        statuses = [status.SINGLE, status.JOINT, status.HEAD_OF_HOUSEHOLD, status.WIDOW, status.SEPARATE]
        ...
        previous_threshold = select(
            in_each_status,
            [get_previous_threshold(ny_taxable_income, scale.thresholds) for scale in scales],
        )

        next_threshold = select(
            in_each_status,
            [get_next_threshold(ny_taxable_income, scale.thresholds) for scale in scales],
        )

        incremental_benefit = select(
            in_each_status,
            [scale.calc(next_threshold + 1) - recapture_base for scale in scales],
        )
        ...
        over_max_agi = ny_agi > sup_tax.agi_limit.max[filing_status]
        main_tax = tax_unit("ny_main_income_tax", period)
        target_tax_if_over_max_agi = ny_taxable_income * sup_tax.agi_limit.tax_rate[filing_status]
