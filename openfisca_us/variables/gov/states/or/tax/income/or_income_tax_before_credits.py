...
class or_income_tax_before_credits(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        return select(
            [filing_status == statuses.SINGLE, filing_status == statuses.JOINT, filing_status == statuses.HEAD_OF_HOUSEHOLD, filing_status == statuses.SEPARATE, filing_status == statuses.WIDOW],
            [rates.single.calc(income), rates.joint.calc(income), rates.head_of_household.calc(income), rates.separate.calc(income), rates.widow.calc(income)])