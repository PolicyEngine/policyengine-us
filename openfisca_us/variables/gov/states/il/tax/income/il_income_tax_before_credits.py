...
class il_income_tax_before_credits(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        recapture_of_investment_credit = tax_unit("recapture_of_investment_credit", period)
        rate = parameters(period).gov.states.il.tax.income.rate

        return max(base_income - exemption_allowance, 0) * rate + recapture_of_investment_credit