...
class snap_utility_allowance(Variable):
    ...
    def formula(spm_unit, period, parameters):
        ...
        lua_due = where(allowance_type == allowance_types.LUA, utility.limited.allowance[region], 0)
        expense_types = utility.single.utility_types
        sum_of_individual_allowances = sum([utility.single[expense.replace("_expense", "")][region] for expense in expense_types])
