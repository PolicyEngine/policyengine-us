...
refundable_payroll_tax_credit = variable_alias("refundable_payroll_tax_credit", rptc)
...
class iitax(Variable):
    ...
    def formula(tax_unit, period, parameters):
        return tax_unit("income_tax_before_refundable_credits", period) - tax_unit(
...
class basic_standard_deduction(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        c15100_if_dsi = max_(
            std.dependent.additional_earned_income + tax_unit("filer_earned", period),
...
class taxbc(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        # Break in worksheet lines
        dwks33 = min_(tax_unit("dwks9", period), tax_unit("filer_e24515", period))
        ...
        for i in range(1, 7):
            ...
            reg_tax += individual_income.bracket.rates[str(i)] * amount_in_bracket
...
class c02900(Variable):
    ...
    unit = USD
    documentation = "Total of all 'above the line' income adjustments to get AGI"
...
class c04470(Variable):
    ...
    unit = USD
    documentation = "Itemized deductions after phase-out (zero for non-itemizers)"
...
class qbided(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        max_qbid = qbinc * qbid.pass_through_rate * qbi_between_threshold_multiplier
...
class c04800(Variable):
    ...
    def formula(tax_unit, period, parameters):
        return max_(0, tax_unit("pre_qbid_taxinc", period) - tax_unit("qbided", period))
...
class c10960(Variable):
    ...
    entity = TaxUnit
    definition_period = YEAR
    documentation = "American Opportunity Credit refundable amount from Form 8863"
    unit = USD
...
class c17000(Variable):
    ...
    unit = USD
    documentation = (
        "Sch A: Medical expenses deducted (component of pre-limitation c21060 total)"
    )
...
class c21040(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        uncapped_phaseout = max_(
            0,
            ((tax_unit("posagi", period) - phaseout.start[mars]) * phaseout.rate),
        )
        return min_(
            uncapped_phaseout,
            phaseout_amount_cap,
...
class c21060(Variable):
    ...
    unit = USD
    documentation = "Itemized deductions before phase-out (zero for non-itemizers)"
...
class c62100(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        separate_addition = max_(
            ...
            min_(
                amt.exemption.amount[mars],
                amt.exemption.phaseout.rate * (c62100 - amt.exemption.separate_limit),
            ),
...
class invinc_agi_ec(Variable):
    ...
    unit = USD
    documentation = "Always equal to zero (will be removed in a future version)"
    definition_period = YEAR
...
class ymod1(Variable):
    ...
    def formula(tax_unit, period, parameters):
        ...
        investment_income = add(tax_unit, period, investment_income_sources) + tax_unit(
            "c01000", period
        )
        BUSINESS_INCOME_SOURCES = ["filer_e00900", "filer_e02000"]
        business_income = add(tax_unit, period, BUSINESS_INCOME_SOURCES)
        max_business_losses = parameters(period).irs.ald.misc.max_business_losses[
            tax_unit("mars", period)
        ]
        business_income_losses_capped = max_(business_income, -max_business_losses)
        return direct_inputs + investment_income + business_income_losses_capped
